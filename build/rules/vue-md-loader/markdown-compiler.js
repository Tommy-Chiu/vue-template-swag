...
const renderVueTemplate = function (html, wrapper) {
  ...
  const output = {
    style: $.html('style'),
    html: null,
    script: $.html($('script').first()) // get only the first script child. Causes issues if multiple script files in page.
  }
  $('style').remove()
  $('script').remove()

  if ($.html().indexOf('<template lang="pug">') !== -1) {
    output.html = `<template lang="pug">\n<${wrapper}>\n${$.html()}\n</${wrapper}>\n</template>`
  } else {
